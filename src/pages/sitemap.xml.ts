...
import { listAllPublicPosts, listPublicTags } from '../lib/posts';
import { listPublishedProjects } from '../lib/db';

export const GET: APIRoute = ({ url }) => {
  const paths = ['/', '/about/', '/blog/', '/projects/', ...listPublishedProjects().map((project) => `/projects/${project.slug}/`), ...listAllPublicPosts().map((post) => `/blog/${post.slug}/`), ...listPublicTags().map(({ tag }) => `/blog/tags/${encodeURIComponent(tag)}/`)];
